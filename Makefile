...
rpm-el7:
	...
	docker run --rm -it \
		...
		-v $$(pwd)/99-DataSourceVMwareGuestInfo.cfg:/root/rpmbuild/BUILD/etc/cloud/cloud.cfg.d/99-DataSourceXenGuestInfo.cfg:ro \
		-v $$(pwd)/DataSourceVMwareGuestInfo.py:/root/rpmbuild/BUILD/usr/lib/python2.7/site-packages/cloudinit/sources/DataSourceXenGuestInfo.py:ro \
		-v $$(pwd)/dscheck_VMwareGuestInfo.sh:/root/rpmbuild/BUILD/usr/bin/dscheck_XenGuestInfo:ro \
		rpmbuild:el7 \
