...
class DataSourceXenGuestInfo(sources.DataSource):
    ...
    def __init__(self, sys_cfg, distro, paths, ud_proc=None):
        ...
        if not get_data_access_method():
            LOG.error("Failed to find xenstore-read")
    ...
    def get_data(self):
        ...
        if not data_access_method:
            LOG.error("xenstore-read is required to fetch guestinfo value")
            return False
        ...
        if data_access_method == XENSTORE_READ and CLEANUP_GUESTINFO in self.metadata:
...
def get_guestinfo_value(key):
    ...
    if data_access_method == XENSTORE_READ:
        try:
            (stdout, stderr) = subp(
                [XENSTORE_READ, "vm-data/" + key])
            if stderr == (NOVAL+key):
                LOG.debug("No value found for key %s", key)
            elif not stdout:
...
def set_guestinfo_value(key, value):
    ...
    if data_access_method == XENSTORE_READ:
        try:
            subp([XENSTORE_WRITE, ("vm-data/%s" % (key)), ("%s" % (value))])
            return True
        except ProcessExecutionError as error:
            util.logexc(
...
def guestinfo(key):
    ...
    return decode('vm-data/' + key, enc_type, data)
...
def get_data_access_method():
    ...
    if XENSTORE_READ:
        return XENSTORE_READ
    return None


_MERGE_STRATEGY_ENV_VAR = 'CLOUD_INIT_XEN_GUEST_INFO_MERGE_STRATEGY'
